...
import numpy as np
import quantus
from resnet101 import ResNetAudio101  # Assuming ResNetAudio is defined in resnet.py
...
def plot_faithfulness_scores(scores):
    ...
    plt.title('Density Plot of Faithfulness Scores for 50 Samples from ResNet101')
    plt.xlabel('Faithfulness Score')
    plt.ylabel('Density')
    plt.grid(True)
    plt.savefig("faithfulness_density101.png")
...
def main():
    ...
    sample_data = np.load("Explanations101_batch.npy", allow_pickle=True).item()
    ...
    model = ResNetAudio101(num_classes=10)  # Adjust num_classes as needed
    ...
    # Calculate uncertainty (assuming it's also adjusted if needed)
    #mean_uncertainty, std_uncertainty = calculate_uncertainty(model, x_batch, device)
    #print("Mean Uncertainty:", mean_uncertainty)
    #print("Standard Deviation of Uncertainty:", std_uncertainty)
