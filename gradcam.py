...
for i, (inputs, labels) in enumerate(test_dataloader):
    ...
    for j, target_label in enumerate(target_labels):
        ...
        if j == 0:
            ...
            ax2.set_title(f'Original MFCC of Class: {class_names[labels[j].item()]}')
            ax2.axis('off')
            ...
            ax3.set_title(f'Overlayed Heatmap on MFCC, Predicted class: {class_names[target_label]}')
            ax3.axis('off')
            ...
            plt.savefig(f"gradcam_explanations_18{i * BATCH_SIZE + j + 1}.png", bbox_inches='tight')
            plt.show()
...
np.save("Explanations18_batch.npy", explanations_dict)
